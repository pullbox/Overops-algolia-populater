...
(async () => {
  ...
  // Queue a request
  await crawler.queue('https://doc.overops.com/reference');
...
function deletelugsMongodb() {
  ...
  mongoclient.connect(mongoUrl, function(err, db) { 
    ...
    dbo.collection("APIs").drop(function(err, delOK) {
        if (err) throw err;
...
//    dbo.collection("slugs").insertOne(JSON.stringify(createJSON(pageurl, hostname, slug)), function(err, res) {
   ...
   //   dbo.collection("slugs").insertOne(createJSON(pageurl, hostname, slug), function(err, res) {
    ...
    dbo.collection("APIs").updateOne( query, { $set: createJSON(pageurl, hostname, slug) }, { upsert: true }, function(err, res) {
        if (err) {
            console.error(err);
            throw err;
        }
